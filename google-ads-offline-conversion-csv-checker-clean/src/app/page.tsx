...
import { defaultDescription, defaultTitle, organizationId, siteName, siteUrl, webApplicationId, websiteId } from "@/lib/site";
...
const workflowPaths = [
  {
    title: "Click-ID upload",
    body: "Use this path when rows contain GCLID, GBRAID, or WBRAID values and the file is intended for a click-based offline conversion import.",
    href: "/guide/gclid-gbraid-wbraid-offline-conversion-csv",
  },
  {
    title: "Enhanced conversions user data",
    body: "Use this path when rows contain email, phone, name, street address, consent fields, or SHA-256 hash-like values for user-provided data review.",
    href: "/guide/enhanced-conversions-for-leads-csv-errors",
  },
  {
    title: "Template and upload readiness",
    body: "Use this path when you need a final checklist for headers, conversion names, timestamps, values, currencies, Order IDs, and duplicate rows.",
    ...
    <main className="min-h-screen">
      ...
      <footer className="mx-auto flex max-w-7xl flex-col gap-3 border-t border-slate-200 px-6 py-8 text-sm text-slate-600 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap gap-4">
          ...
          <a href="/methodology" className="font-medium text-blue-700 hover:text-blue-900">Methodology</a>
...
function StaticSeoContent() {
  return (
    <section className="mx-auto max-w-7xl px-6 pb-16" aria-labelledby="csv-checker-guide">
      ...
      <section className="mt-8 rounded-3xl border border-blue-200 bg-blue-50 p-6 shadow-sm" aria-labelledby="workflow-paths">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide text-blue-700">Workflow first</p>
            <h2 id="workflow-paths" className="mt-2 text-2xl font-bold text-slate-950">Choose the right CSV review path</h2>
          ...
          <a href="/methodology" className="inline-flex rounded-full bg-blue-700 px-5 py-3 text-sm font-semibold text-white hover:bg-blue-800">Read methodology</a>
        </div>
        <div className="mt-5 grid gap-4 md:grid-cols-3">
          {workflowPaths.map((workflow) => (
            <a key={workflow.href} href={workflow.href} className="rounded-2xl border border-blue-100 bg-white p-5 transition hover:border-blue-300 hover:shadow-soft">
              <h3 className="font-bold text-slate-950">{workflow.title}</h3>
              <p className="mt-2 text-sm leading-6 text-slate-700">{workflow.body}</p>
            </a>
          ))}
        </div>
      </section>

      <section className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm" aria-labelledby="methodology-preview">
        <h2 id="methodology-preview" className="text-2xl font-bold text-slate-950">Methodology at a glance</h2>
        <p className="mt-3 leading-7 text-slate-700">
        ...
        <ol className="mt-5 grid gap-3 text-sm leading-6 text-slate-700 md:grid-cols-4">
          {methodologySteps.map((step, index) => (
            <li key={step} className="rounded-2xl bg-slate-50 p-4">
              <span className="text-xs font-semibold uppercase tracking-wide text-blue-700">Step {index + 1}</span>
