...
export function checkIdentifiers(row: Record<string, string>, mapping: FieldMapping, mode: ValidationMode, rowNumber: number, issues: ValidationIssue[]) {
  ...
  if (!hasAnyUsableIdentifier) {
    issues.push(issue("EMPTY_IDENTIFIER", "critical", "No usable identifier value was found in this row.", "Add GCLID, GBRAID, WBRAID, Email, Phone, or complete user-provided address data with first name, last name, street address, country, and zip/postal code for the selected workflow.", { rowNumber }));
  }

  if (hasAnyAddress && !hasUsableAddress) {
    const severity = hasValidClickId || hasValidUserIdentifier ? "warning" : "critical";
    issues.push(issue("INCOMPLETE_ADDRESS_IDENTIFIER", severity, "Address-style data is present, but it is not enough to act as a usable identifier.", "Use address data together with first name, last name, street address, country, and zip/postal code. Location-only values such as country or zip are not a user identifier by themselves.", { rowNumber }));
  }
  ...
  if ((mode === "user_data_preflight" || mode === "user_data_scheduled_prehashed" || mode === "user_data_manual_unhashed") && !hasUserIdentifier && !hasUsableAddress) {
    issues.push(issue("MISSING_USER_DATA_IDENTIFIER", "critical", "This user-data style row has no email, phone, or complete address identifier value.", "Add email, phone, or complete address-style user data with first name, last name, street address, country, and zip/postal code that matches your Google Ads import workflow.", { rowNumber }));
    ...
    issues.push(issue("NO_VALID_USER_DATA_IDENTIFIER", "critical", "This user-data style row has user identifier text, but no valid email, phone, or complete address identifier.", "Fix invalid email or phone values, or provide complete address-style user data with first name, last name, street address, country, and zip/postal code.", { rowNumber }));
  }
