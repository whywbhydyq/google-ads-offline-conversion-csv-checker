...
- `/llms.txt` - AI crawler and citation summary
- `/terms` - terms of use
...
- Gmail/Googlemail normalization reminders, non-E.164 phone values, and uppercase SHA-256 hash formatting
- Invalid SHA-256 hash-like values and incomplete or location-only address rows
- Invalid conversion value, high-precision decimal values, and ISO 4217 currency risks
...
Vercel Analytics is used for high-level product events only. Events use buckets, fixed sample IDs, workflow labels, filter labels, and status labels, not uploaded file names, CSV row content, raw values, search query text, email addresses, phone numbers, click IDs, order IDs, conversion names, conversion values, or other conversion data.
...
- Uploaded CSV checks use a browser Web Worker when possible. Main-thread fallback is limited to browsers that cannot create the worker; worker timeout or runtime failures stop safely instead of re-parsing large files on the UI thread.
...
- Sitemap `lastModified` uses a stable content date instead of the current request time.
- Global Organization and WebSite JSON-LD are emitted from the root layout; page-specific JSON-LD stays on each page.
...
Included: CSV upload, local parsing, field auto-detection, split rule-based validation, stricter user-data normalization checks, results summary, issue table, report download, dynamic sample files, Web Worker validation, FAQ, trust pages, SEO guide pages, stable sitemap metadata, llms.txt, anonymous usage events, and manual validation coverage.
