...
| `sample_loaded` | User loads an in-page sample | `sample` | No |
| `sample_downloaded` | User downloads a generated sample | `sample`, `source` | No |
...
Never send filenames, CSV row content, email addresses, phone numbers, click IDs, order IDs, conversion names, raw values, or search query text to analytics.
