...
export function isGmailOrGooglemail(value: string) {
  const domain = getEmailDomain(value);
  return domain === "gmail.com" || domain === "googlemail.com";
}

export function needsGmailNormalization(value: string) {
  const localPart = value.trim().toLowerCase().split("@")[0] ?? "";
  return isGmailOrGooglemail(value) && (localPart.includes(".") || localPart.includes("+"));
}
...
export function isValidClickId(value: string) {
  const trimmed = value.trim();
  return trimmed.length >= 10 && trimmed.length <= 200 && /^[A-Za-z0-9_-]+$/.test(trimmed);
}
